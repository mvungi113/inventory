...
class _SalesPageState extends State<SalesPage> {
  ...
  Future<void> _addProductToCart(String barcode) async {
    ...
    final productSnap = await _db.child('user_inventory/$userId/products/$barcode').get();
  ...
  void _updateQuantity(String barcode, int newQty) {
    ...
    _db.child('user_inventory/$userId/products/$barcode').get().then((snap) {
      ...
      final availableQuantity = int.tryParse(productData['quantity'].toString()) ?? 0;
  ...
  Future<void> _finalizeSale() async {
    ...
    for (final barcode in cart.keys) {
      ...
      if (soldQty > currentQty) {
        _showMessage('Insufficient stock for product ${cart[barcode]['name']}!');
        return;
      }
    }
    ...
    _showMessage('Sale completed! Payment received: ${totalAmount.toStringAsFixed(2)}');
    ...
    return ListView(
      ...
      children: cart.entries.map((entry) {
        ...
        return ListTile(
          ...
          subtitle: Text(
              'Unit Price: ${product['price']} | Subtotal: ${product['subtotal'].toStringAsFixed(2)}'),
          trailing: SizedBox(
            width: 180,
            child: Row(
